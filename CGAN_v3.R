...
g_embed <- mx.symbol.Embedding(data = label, input_dim = 10, output.dim = embed_size, name = "g_embed")
g_embed <- mx.symbol.reshape(data = g_embed, shape = c(1, 1, embed_size, batch_size), name = "g_reshape")

gen_noise <- mx.symbol.normal(loc=0, scale=1, shape=c(1, 1, noise_size, batch_size), name="gen_rand")
gen_concat <- mx.symbol.concat(data = c(gen_noise, g_embed), num.args = 2, name="gen_concat")

g0 <- mx.symbol.Convolution(data=gen_concat, kernel=c(2,2), stride=c(1,1), pad=c(1,1), num_filter=gen_features*4, no_bias=no_bias, name='g0_conv')
...
g1 <- mx.symbol.Convolution(data=g1, kernel=c(3,3), stride=c(1,1), pad=c(1,1), num_filter=gen_features*3, no_bias=no_bias, name='g1_conv')
...
d_embed <- mx.symbol.Embedding(data = digit, input_dim = 10, output.dim = embed_size, name = "d_embed")
d_embed <- mx.symbol.reshape(data = d_embed, shape = c(1, 1, embed_size, batch_size), name = "d_reshape")
d_embed <- mx.symbol.broadcast_to(data=d_embed, shape=c(28, 28, embed_size, batch_size), name="d_broadcast")

data_concat <- mx.symbol.concat(list(data, d_embed), num.args = 2, dim = 1, name='d_concat')

d1 <- mx.symbol.Convolution(data=data_concat, name='d1', kernel=c(3,3), stride=c(1,1), pad=c(0,0), num_filter=32, no_bias=no_bias)
d1 <- mx.symbol.BatchNorm(d1, name='dbn1', fix_gamma=fix_gamma, eps=eps)
d1 <- mx.symbol.relu(d1, name='dact1')
d1 <- mx.symbol.Pooling(d1, kernel = c(2,2), stride = c(2,2), pool_type = "max")

d2 <- mx.symbol.Convolution(d1, name='d2', kernel=c(3,3), stride=c(1,1), pad=c(0,0), num_filter=64, no_bias=no_bias)
d2 <- mx.symbol.BatchNorm(d2, name='dbn2', fix_gamma=fix_gamma, eps=eps)
d2 <- mx.symbol.relu(d2, name='dact2')

d3 <- mx.symbol.Convolution(d2, name='d3', kernel=c(3,3), stride=c(2,2), pad=c(1,1), num_filter=128, no_bias=no_bias)
d3 <- mx.symbol.BatchNorm(d3, name='dbn3', fix_gamma=fix_gamma, eps=eps)
d3 <- mx.symbol.relu(d3, name='dact3')
# d3 <- mx.symbol.Pooling(d3, kernel = c(2,2), stride = c(2,2), pool_type = "max")

d4 <- mx.symbol.Convolution(d3, name='d4', kernel=c(3,3), stride=c(1,1), pad=c(0,0), num_filter=256, no_bias=no_bias)
d4 <- mx.symbol.BatchNorm(d4, name='dbn4', fix_gamma=fix_gamma, eps=eps)
d4 <- mx.symbol.relu(d4, name='dact4')

# d5 <- mx.symbol.Convolution(d4, name='d5', kernel=c(3,3), stride=c(1,1), pad=c(0,0), num_filter=256, no_bias=no_bias)
# d5 <- mx.symbol.BatchNorm(d5, name='dbn5', fix_gamma=fix_gamma, eps=eps)
# d5 <- mx.symbol.relu(d5, name='dact5')

d6 <- mx.symbol.Convolution(d4, name='d5', kernel=c(4,4), stride=c(1,1), pad=c(0,0), num_filter=1)
# d5 <- mx.symbol.BatchNorm(d5, name='dbn5', fix_gamma=fix_gamma, eps=eps)
# d5 <- mx.symbol.relu(d5, name='dact5')

dflat <- mx.symbol.Flatten(d6, name="dflat")

# dfc <- mx.symbol.FullyConnected(data=dflat, name="dfc", num_hidden=1, no_bias=F)
D_sym <- mx.symbol.LogisticRegressionOutput(data=dflat, label=fake_flag, name='D_sym')
...
g4$infer.shape(list(label = batch_size))
D_sym$infer.shape(list(data = input_shape_D, digit = batch_size))
...
optimizer_G <- mxnet:::mx.opt.adadelta(rho = 0.90, epsilon = 1e-8, wd = 0, rescale.grad = 1, clip_gradient = 1)
optimizer_D <- mxnet:::mx.opt.adadelta(rho = 0.90, epsilon = 1e-8, wd = 0, rescale.grad = 1, clip_gradient = 1)

# optimizer_G <- mxnet:::mx.opt.adam(learning.rate = 1e-4, beta1 = 0.9, beta2 = 0.999, wd = 0, rescale.grad = 1, clip_gradient = 1)
# optimizer_D <- mxnet:::mx.opt.adam(learning.rate = 1e-4, beta1 = 0.9, beta2 = 0.999, wd = 0, rescale.grad = 1, clip_gradient = 1)
