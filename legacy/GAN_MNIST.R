...
d1 = mx.symbol.Convolution(data, name='d1', kernel=c(3,3), stride=c(1,1), pad=c(1,1), num_filter=24, no_bias=no_bias)
...
pool1 <- mx.symbol.Pooling(data=dact1, name="pool", pool_type="max", kernel=c(2,2), stride=c(2,2), pad=c(0,0))

d2 = mx.symbol.Convolution(pool1, name='d2', kernel=c(4,4), stride=c(2,2), pad=c(1,1), num_filter=32, no_bias=no_bias)
...
# Batch on Detective from real data (label=1)
# Batch on Detective from Generator data (label=0)
# Batch on Generator using random input
